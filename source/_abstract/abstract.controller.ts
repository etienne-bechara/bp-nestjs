...
import { AbstractControllerMethod, AbstractControllerOptions } from './interfaces/abstract.controller.options';
...
export class AbstractController<Entity> extends AbstractProvider {
   ...
   */
  @Get()
  public async getEntities(@Query() query: DeepPartial<Entity> & AbstractPartialDto): Promise<Entity[] | AbstractPartialEntity<Entity>> {
    await this.checkImplementation('get');

    const { data, partial } = await this.plainToDtoOffset(query, this.options.dto.read);
   ...
   */
  @Post()
  public async postEntity(@Param() params: DeepPartial<Entity>, @Body() body: DeepPartial<Entity>): Promise<Entity> {
    await this.checkImplementation('post');
    if (!body) throw new BadRequestException(this.MISSING_BODY_MESSAGE);

    let dto = await this.plainToDto(body, this.options.dto.create);
    dto = { ...params, ...dto };
    await this.unflatten(dto);
